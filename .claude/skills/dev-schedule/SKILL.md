---
name: dev-schedule
description: "DevByAlex ops skill for setting up or tearing down the unattended schedule that drives /dev-autopilot so an app keeps progressing toward launch-ready on a cadence. Preflights the gates and the explicit working branch the build runs off of (e.g. staging, never assumed main; a cron has no current-branch intent) and provisioning, then picks a tier (cloud remote routine, local durable cron, or in-session loop) and creates the schedule. The whole workflow, including native skills, reused library skills, and the best-practice knowledge/, is vendored into the app's committed .claude by install.sh, so a runner needs no MCP token or network brain; a committed checkout is fully self-sufficient. Use when the user says 'schedule the autopilot', 'set up the scheduled task', 'run the build unattended', or 'create the cron/routine'."
